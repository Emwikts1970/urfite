...
data(CASchools) 
CASchools$size  <- CASchools$students/CASchools$teachers     # teacher-student-ratio
...
model <- lm(score ~ size + english + FracEL, data = CASchools)
summary(model)
```
</div>
...
model <- lm(score ~ size + english + NVS, data = CASchools)
summary(model)
```
</div>
...
model <- lm(score ~ size + english + PctES, data = CASchools)
summary(model)
```
</div>
