...
If we use a simple linear regression model we assume that the relationship between the two variables can be represented by a
straight line ($y = mx + n$). Lets suppose that the "true" function which relates test scores and student to teacher ratio is

$$TestScore = 713 - 3 \times ClassSize.$$

Now we use the `plot()` function to produce a scatterplot of the data from above. Furthermore, we add the line using the function `abline()`. 

```{r}
plot(TestScore ~ StudentTeacherRatio, data = class_data)
abline(a = 713, b = -3)
```
...
In order to account for the differences we extend the regression model by an **error term** which covers these random effect. In general the **error term** accounts for all differences between the "true" regression line and the actual observed data. This could next to the pure randomness also be measerment errors or the effect of important but not considered independent variables.   
...
- $Y_i$ is the *dependent variable*, the *regressand*Library, or simply the *left-hand variable*
...
We find that the dataset consists of plenty variables but the two we are intersted in (average test scores and student-teacher ratio) are not included. However, it is possible to calculate both from the existing data. To obtain the student to teacher-ratio we divide the number of students by the number of teachers. The avarage test score is the arithmetic mean of the test score for reading and the score of the math test.  
...
The next code chunk reproduces table 4.1 from the text book which summarizes the distribution of test scores and class sizes. In order to have a nice display format we gather all data after the computation in a data frame. 
...
DistributionSummary <- data.frame(Average           = c(avg_tsratio, avg_score), 
                                  StandardDeviation = c(sd_tsratio, sd_score), 
...
Before we start with the regression it is a good idea to take a closer look at our data set by plotting it. This allows us to detect specific characteristics of our data, such as outliers, which are hard to discover by looking at mere numbers. The easiest way to do so in `R` is the function `plot()`.   
Figure 4.2 shows a scatterplot of all observations. We see that the points are scatterd strongly and a apparent relationship cannot be detected with the naked eye.
...
The task we are facing now is to find the line which fits best to this data.  
...
There are many possible ways to compute $\left(\hat{\beta_0}\right)$ and $\left(\hat{\beta_1}\right)$ in R. We could implement the formulas by two of R's most basic functions `mean()` and `sum()`. But of course their are also other and even more manual ways to do the same tasks. 
...
```{r, results='hold',fig.align='center', fig.cap='Figure 4.4'}
...
that is, $\hat{\beta_0}$ and $\hat{\beta_1}$ are unbiased estimators of $\beta_0$ and $\beta_1$. If the sample is sufficiently large, by the central limit theorem the sampling distribiution of the estimators is well approximated by the bivariate normal distribution. This implies that the marginal distributions are also normal in large samples.
...
What does this tell about the significance of the estimated coefficient? Using R we may visualise how such a statement can be made:
...
# Compute both estimates 1e5 times and store the result in vectors  
...
Testing joint hypothesis in a multiple regression framework.
