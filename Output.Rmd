...
If you use `lm` to estimate a model with a set of regressors that suffer from perfect multicollinearity the system will warn you and ignore the regressor(s) which is(are) assumed to be a linear combination of the others.
```{r}
# Example 1
library(AER)                                                    # contains the dataset 
data(CASchools) 
...
CASchools$tsratio  <- CASchools$students/CASchools$teachers     # teacher-student-ratio
CASchools$score    <- (CASchools$read + CASchools$math)/2       # average test-score
CASchools$NVS      <- ifelse(CASchools$tsratio < 12, 0, 1)      # If tsration smaller 12, NVS = 0, else NVS = 1
