...
You can easily see this by inspection of the summary the regression model
...
The outputs above tell us that 0 is not an element of the computed confidence intervals for the coefficient of `tsratio` such that we can reject the null hypothesis at significance levels of 5\% and 10\% (Note that rejection at the 5\%-level implies rejection at the 10\% level. Why?). The same conclusion can be made when beholding the $p$-value for `tsratio`: $0.00398 < 0.05 = \alpha$.
...
$$ TestScore = \beta_0 + \beta_1 \times STR + \beta_2 \times english + \beta_3 \times Expn + u $$
...
model <- lm(score ~ tsratio + english + expenditure, data = CASchools)
summary(model)
```

We see that the estimated effect of a change in the student-teacher ratio on test scores with expenditures per pupil and the share of english learning pupils held constant is rather small (`-0.29`). What is more, the coefficient of `tsratio` is not significantly different from zero anymore ($p\text{-value}=$`0.55) so the new model provides no evidence that changing the student-teacher ratio has any effect on the test scores in this case.
...
Luckily, it is fairly easy to conduct an $F$-test in R
...
model <- lm(score ~ tsratio + english + lunch, data = CASchools)
summary(model)
```
</div>
...
$$ \widehat{TestScore} = \underset{(4.7)}{700.2} - \underset{(0.24)}{1.00} \times STR - \underset{(0.032)}{0.122} \times english + \underset{(0.022)}{0.547} \times lunch. $$

We observe no substantial changes in the conclusion about the effect of `STR`: the coefficient changes by only $0.1$ and keeps its significance. 
