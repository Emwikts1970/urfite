...
To start with an easy example consider the following combinations of average test scores and the average student to teacher ratios in some school districts:
...
Lets suppose that the "true" function which relates test scores and student to teacher ratio 
to each other is
...
For our purpose we can use the function `plot()` to produce a scatterplot with `StudentTeacherRatio` on the $X$-axis and `TestScore` on the $Y$-axis. An easy way to do so is to call `plot(y_variable ~ x_variable)`.   
...
`class(object_name)` returns the type (class) of the object. Depending on the class of an object different
...
With `head()` we get a first overview over our data. This function shows only the first 6 rows of the dataset which prevents an overcrowded console output. An alternative to `class()` and `head()` is `str()` which means structure and gives an comprehensive overview of the object.  
