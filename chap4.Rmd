...
```{r ex="create_a", type="pre-exercise-code", tut=T}
library(AER)                                                    # contains the data set 
data(CASchools) 
CASchools$size     <- CASchools$students/CASchools$teachers     # class size
CASchools$score    <- (CASchools$read + CASchools$math)/2       # average test-score
...
```{r ex="create_a", type="sample-code", tut=T}
#apply summary to the CASchools data.frame

```

```{r ex="create_a", type="solution", tut=T}
#apply summary to the CASchools data.frame
summary(CASchools)
```

```{r ex="create_a", type="sct", tut=T}
test_output_contains(summary(CASchools))
